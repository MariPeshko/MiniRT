...
int	parse_ambient_lighting(char *line)//+ large struct
{
	...
	//get RGB value from arguemnts [2]
	free(arguments);
	return (0);
}
...
int	parse_delegate(char *line)//+ largest struct
{
	...
	if (ft_strncmp(line, "C", 1) == SUCCESS)
		return (parse_specific(line));//+ largest struct
	if (ft_strncmp(line, "L", 1) == SUCCESS)
		return (parse_specific(line));//+ largest struct
	if (ft_strncmp(line, "cy", 2) == SUCCESS)
		return (parse_specific(line));//+ largest struct
	if (ft_strncmp(line, "pl", 2) == SUCCESS)
		return (parse_specific(line));//+ largest struct
	if (ft_strncmp(line, "sp", 2) == SUCCESS)
		return (parse_specific(line));//+ largest struct
	return (SUCCESS);
}
